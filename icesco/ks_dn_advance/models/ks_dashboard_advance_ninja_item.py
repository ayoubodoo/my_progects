...
class KsDashboardNinjaItemAdvance(models.Model):
    ...
    def ks_format_query_result(self, ks_query_result):
        ks_list_view_data = {'label': [], 'data_rows': [], 'date_index': [], 'type': 'query','fields_type':[]}
        query_result = json.loads(ks_query_result)
        if query_result:
            if query_result.get('ks_date_header_len',False):
                ks_list_view_data['date_index'] = query_result.get('ks_date_header_len', False)
            if query_result.get('ks_field_type', False):
                ks_list_view_data['fields_type'] = query_result.get('ks_field_type', False)
    ...
    def ks_run_query(self):
        ...
        if self.ks_query_end_date:
            ...
            if self.ks_custom_query and (
                    ("%(ks_start_datetime)" in self.ks_custom_query) or ("%(ks_end_datetime)" in self.ks_custom_query)):
                ks_type = 'datetime'
        ...
        for rec in self:
            ...
            if rec.ks_data_calculation_type == 'query' and rec.ks_dashboard_item_type not in ['ks_tile', 'ks_kpi'] \
                ...
                if rec.ks_dashboard_item_type == 'ks_list_view':
                    ...
                    new_env = self.env
                    if self.ks_validate_query(ks_start_date, ks_end_date, ks_query, start_date, end_date, new_env):
    ...
    def ks_validate_query(self, ks_start_date, ks_end_date, ks_query, start_date, end_date, new_env):
        try:
            if self.ks_dashboard_item_type != 'ks_list_view':
                if self.ks_is_date_ranges:
                    new_env.cr.execute("with ks_chart_query as (" + ks_query + ")" +
                                       "select * from ks_chart_query limit %(ks_limit)s",
            ...
            if type(e).__name__ == 'KeyError':
                raise ValidationError(
                    _('Wrong date variable' + ' ' + e.args[
                        0] + ',' + 'Please use ks_start_date and ks_end_date or ks_start_datetime and ks_end_datetime '
                                   'in custom query'))
            raise ValidationError(_(e))


    def ks_list_validate_query(self, ks_start_date, ks_end_date, ks_query, start_date, end_date, new_env, ks_offset,
                               limit, ks_export_all):
        try:
            if self.ks_dashboard_item_type == 'ks_list_view':
                if self.ks_is_date_ranges:
            ...
            if type(e).__name__ == 'KeyError':
                raise ValidationError(
                    _('Wrong date variable' + ' ' + e.args[
                        0] + ',' + 'Please use ks_start_date and ks_end_date or ks_start_datetime and ks_end_datetime in custom query'))
            raise ValidationError(_(e))
    ...
    def ks_get_list_query_result(self, ks_query, selected_start_date, selected_end_date, ks_offset=0,
        ...
        if self.ks_is_date_ranges:
            ...
            if self.ks_custom_query and (("%(ks_start_datetime)" in self.ks_custom_query) or (
                    "%(ks_end_datetime)" in self.ks_custom_query)):
                ks_type = 'datetime'
                ks_start_date = 'ks_start_datetime'
                ks_end_date = 'ks_end_datetime'
        type_code = []
        ...
        if ks_query and "{#UID}" in ks_query:
            ks_query = ks_query.replace("{#UID}", str(self.env.user.id))
        ks_lst_validate = self.ks_list_validate_query(ks_start_date, ks_end_date, ks_query, start_date, end_date,
                                                      new_env, ks_offset, limit, ks_export_all)
        if ks_lst_validate:
            if ks_export_all:
                if self._context.get('ksDateFilterSelection', False):
                    ksDateFilterSelection = self._context.get('ksDateFilterSelection', False)
            ...
            if records:
                ...
                for header_key in header:
                    if type(records[0][header_key]).__name__ == 'float' or \
                        ...
                        type_code.append('string')
                    ks_field_type.append(type(records[0][header_key]).__name__)

        if records:
            for key in records[0]:
                if type(records[0][key]).__name__ == 'datetime' or type(records[0][key]).__name__ == 'date':
                    ks_lst_index.append(list(records[0].keys()).index(key))
        ...
        return json.dumps({'header': header,
                           'records': records, 'type_code': type_code,
                           'ks_is_group_column': False,
                           'ks_date_header_len': ks_lst_index if ks_lst_index else [],
                           'ks_field_type' : ks_field_type
                           })
