...
class PortalLeaveKnk(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        values = super(PortalLeaveKnk, self)._prepare_portal_layout_values()
        employee = request.env.user.employee_id
        show = False
        if req.env.user.has_group('portal_leave_knk.icesco_conge_portal'):
            show = True
        if request.env.user._is_admin():
            domain = []
        else:
            domain = [('employee_id', '=', employee.id)]
        orders_count = req.env['hr.leave'].sudo().search_count(
            domain)
        values['show'] = show
