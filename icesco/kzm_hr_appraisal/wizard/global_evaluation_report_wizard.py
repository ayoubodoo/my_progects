...
class GlobalEvaluationWizard(models.TransientModel):
    _name = 'appraisal.global.wizard'
    _description = 'Appraisal Global Evaluation'
    ...
    model_evaluation_id = fields.Many2one('appraisal.model', "Model")
        ...
        grouped_evaluations = groupby(
            sorted(self.env['appraisal.appraisal'].search(
                [('model_evaluation_id', '=', self.model_evaluation_id.id),
                 ('evaluation_date', '>=', self.start_date),
                 ('evaluation_date', '<=', self.end_date)]), # ('evaluator_id','=',self.env.user.id)
    ...
    def action_print_model(self):
        return self.env.ref(
            'kzm_hr_appraisal.gloabal_evaluation_menu_id').report_action(self)
