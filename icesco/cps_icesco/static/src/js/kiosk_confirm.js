odoo.define('cps_icesco.kiosk_confirm', function (require) {
"use strict";
...
var KioskConfirm = AbstractAction.extend({
    events: {
        ...
        "click .o_hr_attendance_sign_in_out_icon": _.debounce(function () {
            ...
            this._rpc({
                    model: 'hr.attendance',
                    method: 'get_action_normal',
                    args: [""],
                }).then(function (result) {
                    if (window.location.href.includes(result) == true || window.location.href.includes('hr_attendance_kiosk_confirm') == true) {
                            console.log('home normal');
                            self._rpc({
                                    model: 'hr.employee',
            ...
            this._rpc({
                ...
                console.log('bbbbaaa', result);
                console.log('bbbb', window.location.href);
                    if (window.location.href.includes(result) == true || window.location.href.includes('hr_dh_attendance_kiosk_confirm') == true) {
                            console.log('home not normal');
                            self._rpc({
                                    model: 'hr.employee',
                                    method: 'dh_attendance_manual',
                                    args: [[self.employee_id], self.next_action],
                                })
                                .then(function(result) {
                                    if (result.action) {
            ...
            this._rpc({
                    ...
                    if (window.location.href.includes(result) == true || window.location.href.includes('hr_attendance_kiosk_confirm') == true) {
                        ...
                        self._rpc({
                                model: 'hr.employee',
                                method: 'attendance_manual',
                                args: [[self.employee_id], self.next_action, self.$('.o_hr_attendance_PINbox').val()],
                            })
                            .then(function(result) {
                                if (result.action) {
                                    self.do_action(result.action);
...
core.action_registry.add('hr_dh_attendance_kiosk_confirm', KioskConfirm);
