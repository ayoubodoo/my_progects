odoo.define('cps_icesco.greeting_message', function (require) {
"use strict";

var AbstractAction = require('web.AbstractAction');
var attendanceGreeting = require('hr_attendance.greeting_message');
...
var GreetingMessage = AbstractAction.extend({
    ...
    init: function(parent, action) {
        ...
        // if no correct action given (due to an erroneous back || refresh from the browser), we set the dismiss button to return
    ...
    welcome_message: function() {
        ...
        console.log('heeere')
//         ajax.jsonRpc("/check_pointage", 'call', {}).then(function (data) {
//             console.log(data);
//         });
//         ajax.rpc('/web/dataset/call_kw', 'call', {
//             model: 'my.model',
//             method: 'my_method',
//             args: [[]],
//             kwargs: {arg1: value1, arg2: value2},
//         }).then(function (data) {
//             // Do something with the data returned by the Python method
//         });
//         let employee = rpc.query({
//           model: 'hr.employee',
//           method: 'call_fonction_pointage',
//           //context: {},
//           args: [this.attendance],
//             });
//             console.log('teeest', employee);
//             employee.then(function(ribbon_data){
//                 console.log('dadada', ribbon_data)
        ...
        let employee = rpc.query({
          ...
          args: [this.attendance],
            ...
            employee.then(function(result){
                    console.log('dadada', result.warning)
                    if (result.warning) {
                        self.$('.o_hr_attendance_warning_message').show().append(_t("<b>You cannot check in because you arrive too late !</b> please contact the responsible<br/><b>Vous ne pouvez pas faire le pointage car vous êtes arrivé trop tard !</b> Merci de contacter le responsable<br/><b>لا يمكنك تسجيل الوصول لأنك وصلت متأخرًا جدًا</b> يرجى الاتصال بالمسؤول<br/>"));                        self.do_warn(result.warning);
                        clearTimeout(self.return_to_main_menu);
                        self.activeBarcode = false;
                        setTimeout( function() { self.do_action(self.next_action, {clear_breadcrumbs: true}); }, 5000);
                        self._rpc({
                            model: 'hr.attendance',
                            method: 'unlink',
                            args: [self.attendance.id],
                        });
                    } else {
...
//                this.$('.o_hr_attendance_warning_message').show().append(_t("<b>You cannot check in because you arrive too late !</b> please contact the responsible<br/><b>Vous ne pouvez pas faire le pointage car vous êtes arrivé trop tard !</b> Merci de contacter le responsable<br/><b>لا يمكنك تسجيل الوصول لأنك وصلت متأخرًا جدًا</b> يرجى الاتصال بالمسؤول<br/>"));
//                clearTimeout(this.return_to_main_menu);
//                this.activeBarcode = false;
//                setTimeout( function() { self.do_action(self.next_action, {clear_breadcrumbs: true}); }, 1000);
//            }
//        }
            ...
            if(self.previous_attendance_change_date){
                var last_check_out_date = self.previous_attendance_change_date.clone();
...
//                setTimeout( function() { self.do_action(self.next_action, {clear_breadcrumbs: true}); }, 1000);
//            } else
//            if(now - last_check_in_date > 1000*60*60*8){
