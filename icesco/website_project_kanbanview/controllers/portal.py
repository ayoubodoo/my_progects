...
class CustomerPortal(CustomerPortal):
    ...
    def task_kanban_searchbar_task(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None, search_in='content', groupby='project', **kw):
        ...
        project_groups = request.env['project.task'].sudo().read_group([('project_id', 'not in', projects.ids)],
        ...
        tasks = request.env['project.task'].sudo().search(domain, order=order, limit=page_count_kanban, offset=(page - 1) * page_count_kanban)
        request.session['my_tasks_history'] = tasks.ids[:100]
        if groupby == 'project':
            grouped_tasks_list = [request.env['project.task'].sudo().concat(*g) for k, g in groupbyelem(tasks, itemgetter('project_id'))]
        else:
