...
//            ];
            ...
            this.ks_date_filter_selection_order = ['l_custom'];
        ...
        on_attach_callback: function() {
            ...
            $.when(self.ks_fetch_items_data()).then(function(result){
                ...
                if (self.ks_dashboard_data.ks_item_data) {
                    ...
                    odoo.session_info.user_context['gridstack_config'] = self.ks_get_current_gridstack_config();
                }
            });
        },
...
//                            self.grid.addWidget($(item_view), self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 6, null, 2, 2, items[i].id);
        ...
        _ksRenderDashboardTile: function(tile) {
            ...
            var ks_rgba_background_color, dh_ks_rgba_background_color, ks_rgba_font_color, ks_rgba_default_icon_color,ks_rgba_button_color;
            ...
            var def = this._rpc({
                model: 'ks_dashboard_ninja.item',
                method: 'search_read',
                args: [[['id', '=', tile.id]], ['symbol']],
            })
            .then(function (ks_it){
                   return ks_it[0]['symbol'];
                });
            ...
            dh_ks_rgba_background_color = self._ks_get_rgba_format(tile.dh_ks_background_color);
...
//                    var ks_rgba_dark_background_color_l2 = self._ks_get_rgba_format(self.ks_get_dark_color(tile.ks_background_color.split(',')[0], tile.ks_background_color.split(',')[1], -10));
                    style_image_body_l2 = "background-color:" + dh_ks_rgba_background_color + ";";
...
//             var color_set = ['#F04F65', '#f69032', '#fdc233', '#53cfce', '#36a2ec', '#8a79fd', '#b1b5be', '#1c425c', '#8c2620', '#71ecef', '#0b4295', '#f2e6ce', '#1379e7']
            var color_set = ['#008085', '#00ACA8', '#F5BD1E', '#1F5557', '#555555', '#8B8681', '#C29400', '#1c425c', '#8c2620', '#71ecef', '#0b4295', '#f2e6ce', '#1379e7']
            if (palette !== "default") {
        ...
        _renderChart: function($ks_gridstack_container, item) {
            ...
            if (item.ks_data_label_type == 'value' && chart_data.datasets.length >0){
                ...
                plugins: {
                    datalabels: {
                        ...
                        formatter: function(value, ctx) {
                            ...
                            let percentage = sum === 0 ? 0 + "%" : (value * 100 / sum).toFixed(0) + "%";
                            if (item.ks_data_label_type == 'value'){
//                                percentage = value.toFixed(2).replace('.',',');
                                var ks_field = chart_data.ks_field;
                                if (ks_field) {
                                    percentage = value.toFixed(0) + ks_field;

                                } else {
                                   percentage = value.toFixed(0);
                                }
                                percentage = addCommas(percentage);
                            }
                            return percentage;
                        },
                    },
        ...
        ksChartColors: function(palette, ksMyChart, ksChartType, ksChartFamily, stack, semi_circle, ks_show_data_value, chart_data, item) {
            ...
            switch (palette) {
                ...
                case 'warm':
                    gradient = {
                        100: [255, 255, 255, 1],
                        65.001: [255, 255, 255, 1],
                        65: [56, 69, 84, 1],
                        45.001: [56, 69, 84, 1],
                        45: [139, 134, 129, 1],
                        20.001: [139, 134, 129, 1],
                        20: [0, 172, 168, 1],
                        0.001: [0, 172, 168, 1],
                        0: [0, 0, 0, 1]
                    };
                ...
                case 'default':
                    var color_set = ['#008085', '#00ACA8', '#F5BD1E', '#1F5557', '#555555', '#8B8681', '#C29400', '#1c425c', '#8c2620', '#71ecef', '#0b4295', '#f2e6ce', '#1379e7']
//                    var color_set = ['#1F5557', '#C29400', '#008085', '#00ACA8', '#555555', '#8B8681', '#F5BD1E', '#1c425c', '#8c2620', '#71ecef', '#0b4295', '#f2e6ce', '#1379e7']
            }
...
//                    self.$el.find(".grid-stack-item[gs-id=" + item_data.id + "]").append(self.ksRenderToDoDashboardView(item_data));
                }else{
...
//            }
        ...
        ksSum: function(count_1, count_2, item_info, field, target_1, $kpi_preview, kpi_data) {
            ...
            item_info['target_value'] = field.ks_target_value;
            item_info['count_value'] = field.ks_count_value;
            item_info['result_value'] = field.ks_result_value;
        ...
        renderKpi: function(item, grid) {
            ...
            var dh_ks_rgba_background_color = self._ks_get_rgba_format(field.dh_ks_background_color);
...
//            parseInt(Math.round((count_1 / target_1) * 100)) ? field_utils.format.integer(Math.round((count_1 / target_1) * 100)) : "0"
            ...
            var item_info = {
                ...
                target_value: field.ks_target_value,
                count_value: field.ks_count_value,
                result_value: field.ks_result_value,
