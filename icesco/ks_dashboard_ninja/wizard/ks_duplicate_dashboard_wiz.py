...
class KSDuplicateDashboardWizard(models.TransientModel):
    ...
    ks_top_menu_id = fields.Many2one('ir.ui.menu', string="Show Under Menu", required=True,
                                     default=lambda self: self.env['ir.ui.menu'].search(
                                         [('name', '=', 'My Dashboard'), ('parent_id', '=', False)]), domain="[('child_id', '!=', False)]")
    ...
    def ks_duplicate_record(self):
        ...
        active_dash = self.env['ks_dashboard_ninja.board'].browse(dashboard_id).ks_child_dashboard_ids.filtered(
            lambda p: p.ks_active)
