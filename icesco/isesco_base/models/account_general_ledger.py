...
class AccountGeneralLedgerReport(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        return [
            ...
            {'name': _('Balance USD'), 'class': 'number'},
        ]
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        total_total = total_debit = total_credit = total_balance = 0.0
        for account, periods_results in accounts_results:
            ...
            total = account_sum.get('total', 0.0) + account_un_earn.get('total', 0.0)
            ...
            lines.append(self._get_account_title_line(options, account, amount_currency, total, debit, credit, balance, has_lines))
            ...
            if has_lines and (unfold_all or is_unfolded):
                ...
                lines.append(self._get_initial_balance_line(
                    ...
                    account_init_bal.get('total', 0.0) + account_un_earn.get('total', 0.0),
        ...
        for i, options_period in enumerate(options_list):
            ...
            queries.append('''
                SELECT
                    ...
                    SUM(account_move_line.total) AS total,
    ...
    def _get_query_amls(self, options, expanded_account, offset=None, limit=None):
        ...
        query = '''
            SELECT
                ...
                account_move_line.total  AS total,
    ...
    # COLUMN/LINE HELPERS
    ####################################################
    def format_total_value(self, total, currency_id):
        # return round(total, 2)
        return formatLang(self.env, round(total, 2), currency_obj=currency_id)

    @api.model
    def _get_account_title_line(self, options, account, amount_currency, total, debit, credit, balance, has_lines):
        ...
        return {
            ...
            'columns': [
                ...
                {'name': self.format_total_value(total, self.env.ref('base.USD')), 'class': 'number'},
    ...
    @api.model
    def _get_initial_balance_line(self, options, account, amount_currency,total, debit, credit, balance):
        has_foreign_currency = account.currency_id and account.currency_id != account.company_id.currency_id or False
        return {
    ...
    @api.model
    def _get_account_total_line(self, options, account, amount_currency, total, debit, credit, balance):
        has_foreign_currency = account.currency_id and account.currency_id != account.company_id.currency_id or False
        return {
    ...
    @api.model
    def _get_total_line(self, options, total, debit, credit, balance):
        return {
